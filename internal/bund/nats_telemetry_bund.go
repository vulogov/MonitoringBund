...
func bundNatsSendLog(l *tc.TCExecListener, v interface{}) interface{} {
	switch data := v.(type) {
	case *tc.TCJson:
		err := NatsSendLog(data)
		if err != nil {
			l.TC.SetError(fmt.Sprintf("[ LOG ] %v", err))
		}
	}
	return nil
}
...
func BUNDNatsSendLog(l *tc.TCExecListener, name string, q *deque.Deque) (interface{}, error) {
	err := stdlib.ExecuteSingleArgumentFunction(l, "natssendlog", q)
  if err != nil {
    return nil, err
  }
	return nil, nil
...
func init() {
	...
	stdlib.RegisterContextFunctionCallback("natssendlog", tc.Json, bundNatsSendLog)
	tc.SetFunction("log.Send", BUNDNatsSendLog)
}
